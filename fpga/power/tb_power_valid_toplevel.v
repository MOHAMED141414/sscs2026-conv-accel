`timescale 1ns / 1ps
//=============================================================================
// tb_power_valid_toplevel.v  -  SAIF trace against the POST-ROUTE netlist
//
// Drives the full conv_top (UART wrapper + core) via the real UART protocol,
// matching how the board is actually used, so the activity trace reflects
// the complete implemented design -- not just the bare core -- for accurate
// net-name matching against the post-route netlist.
//
// Reset/coef-load excluded from the dump; dump stops right after the last
// result is transmitted (no idle tail).
//=============================================================================
module tb_power_valid_toplevel;
    ...
    localparam NPIX = 80;  // representative window: 2.5 rows, real UART timing
    ...
    // NOTE: post-route netlist top is named "conv_top"; instance name below
    // is "dut" so the SAIF scope matches: /tb_power_valid_toplevel/dut
    conv_top dut (
    ...
    initial begin
        ...
        $display("Post-route power trace complete: %0d pixels through full UART protocol", NPIX);
        $finish;
    end
